	...
	public JSONEntity json2homegoods(JSONObject json) {
		...
		try {
			...
			if (flag) {
				...
				for (int i = 0; i < array.length(); i++) {
					list.add(new Goods(array.getJSONObject(i),1));
				}
				GoodsSet set=new GoodsSet(list);
				entity.setData(set);
			}else {
				entity.setFlag(flag);
				entity.setMessage(json.getInt("msg"));
		...
		try {
			...
			if (flag) {
				...
				for (int i = 0; i < array.length(); i++) {
					...
					Goods goods = new Goods((JSONObject) array.get(i),3);

					//Goods goods = new Goods((JSONObject) array.get(i),0);
		...
		public JSONEntity json2goodsdetail(JSONObject json){
			...
			try {
				...
				if(flag){
					...
					Goods goods;
					if(!json.getString("goods").equals("null")){
						JSONObject goodsdetail=json.getJSONObject("goods");
						goods =new Goods(goodsdetail,1);
						}else{
							goods=new Goods();
						}
						
					JSONObject cardetail=json.getJSONObject("car");
					Car  car=new Car(cardetail,1);
					
					
					JSONObject merchantdetail=json.getJSONObject("merchant");
					Merchant merchant=new Merchant(merchantdetail,1);
						
					
					
					entity.setData(goods);
					entity.setData1(merchant);
					entity.setData2(car);
				}
				else {
					entity.setFlag(flag);
