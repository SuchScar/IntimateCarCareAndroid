	...
	private void setclicklistent() {
			...
			public void onClick(View v) {
				...
				if (!carleixing.getText().toString().equals("")) {
					if (visibility_Flag) {
						...
						new HttpTask(allbrangscallback,MyURL.ALLARCTIC,AddLicenseplateActivity.this).execute(selectcar);
		...
		rel_comment.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(inten.getIntExtra("build",-1)==1){
					  if(tag==1){
						String str="自动挡"; 
					  }
					  else{
					  ...
					  String caunum= city.getText().toString()+edt_carnumber.getText().toString();
					  HashMap<String, String> getinsurancedate = new Car().addnewcar
								 (AddLicenseplateActivity.this,caunum,carleixing.getText().toString(),carname2.getText().toString(),str,tv_pailiang.getText().toString());
				
					 new HttpTask(addnewcarcallback,MyURL.ADDARCTIC,AddLicenseplateActivity.this).execute(getinsurancedate);
				  }
				  if(inten.getIntExtra("build", -1)==2){
...
HttpCallback addnewcarcallback=new HttpCallback() {
	...
	if(entity.isFlag()){
		...
		ToastUtil.show(AddLicenseplateActivity.this, "添加失败");
	}	 
	}
};
